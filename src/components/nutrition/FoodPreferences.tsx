...
const field = "min-h-tap w-full rounded-lg border border-hairline bg-surface px-3 py-2.5 font-body text-base text-ink focus:border-ink";
...
export function FoodPreferences({ pattern, avoid }: { pattern: string; avoid: string }) {
  ...
  return (
    <section aria-label="Food preferences" className="flex flex-col gap-3 rounded-lg border border-hairline bg-surface p-5">
      <div>
      ...
      {state.ok ? (
        <p role="status" className="rounded-lg border border-success bg-surface px-3 py-2 text-sm text-success">Preferences saved.</p>
      ) : null}
