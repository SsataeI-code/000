...
export function LogSuggestionButton({ food }: { food: FoodPick }) {
  ...
  return (
    <button
      ...
      className="min-h-tap shrink-0 rounded-lg border border-hairline bg-surface px-3 py-1 font-label text-[12px] uppercase tracking-wide text-ink hover:border-red hover:text-red disabled:opacity-50"
    >
      {done ? "Added" : "Add"}
    </button>
  );
