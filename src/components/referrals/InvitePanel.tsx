...
export function InvitePanel({
  ...
  if (!code) {
    return (
      <section className="border border-hairline bg-surface p-5">
        <h2 className="text-2xl text-ink">{getCopy("client.invite.title")}</h2>
  ...
  async function share() {
    const shareData = { title: getCopy("brand.name"), text: inviteMessage(link), url: link };
    try {
  ...
  return (
    <section className="border border-hairline bg-surface p-5">
      <h2 className="text-2xl text-ink">{getCopy("client.invite.title")}</h2>
      <p className="mt-2 font-body text-sm text-ink/60">{getCopy("client.invite.body")}</p>
      ...
      <div className="mt-4 flex items-center gap-2 border border-hairline bg-surface-muted px-3 py-2.5">
        ...
        >
          {copied ? getCopy("client.invite.copied") : getCopy("client.invite.copy")}
        </button>
      </div>
      ...
      >
        ...
        {getCopy("client.invite.share")}
      </button>

      {stats.total > 0 ? (
        <div className="mt-4 flex gap-3 border-t border-hairline pt-4">
          <Stat value={stats.joined} label={getCopy("client.invite.joinedLabel")} />
          <Stat value={stats.rewarded} label={getCopy("client.invite.rewardedLabel")} />
        </div>
      ) : null}
    </section>
  );
