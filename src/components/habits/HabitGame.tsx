  ...
  return (
    <section aria-label="Your habit progress" className="border border-hairline bg-elevated p-5 text-white">
      ...
      <div className="flex items-baseline justify-between gap-3">
        <div>
          <p className="font-label text-[10px] uppercase tracking-widest text-white/60">Level {state.level}</p>
          <p className="font-display text-2xl uppercase tracking-wide text-white">{state.levelName}</p>
        </div>
        <span className="inline-flex items-center gap-1.5 font-label text-xs uppercase tracking-wide text-white/80">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Chip
          icon={<span className={currentStreak > 0 ? "text-red" : "text-white/40"}><IconFlame /></span>}
      ...
      <div className="mt-4 border-t border-white/15 pt-4">
        <div className="flex items-baseline justify-between gap-3">
          <p className="font-label text-[10px] uppercase tracking-widest text-white/60">Today</p>
          {perfect ? (
            <span className="animate-red-pulse border border-red bg-red px-2 py-0.5 font-label text-[10px] uppercase tracking-widest text-white">
              Perfect day
            </span>
          ) : (
            <p className="font-body text-xs text-white/70">{todayDone} of {todayDue} done</p>
          )}
        </div>
        <div className="mt-2 h-2 w-full overflow-hidden bg-white/15">
          <div
        ...
        <p className="mt-2 font-body text-sm text-white/85">{line}</p>
      </div>
    </section>
  );
}
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 border px-2.5 py-1 font-label text-[11px] uppercase tracking-wide ${
        accent ? "border-red text-red" : "border-white/25 text-white/80"
      }`}
    >
