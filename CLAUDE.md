...
- **Next:** PWA push (needs VAPID keys), quiet hours.
...
3. **Enable re-engagement email (3/5/7 days):** create a **Resend** account, verify a sender domain, and set `RESEND_API_KEY` (and optionally `EMAIL_FROM`) in Vercel. Without it, in-app nudges + coach alerts still fire; only the email is skipped.
4. *(Later, PWA push — not built yet)* generate Web Push **VAPID keys**.
