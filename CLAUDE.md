...
- **Next:** PWA push + the 3-day re-engagement email (both need owner-provided keys — see *Owner action needed*), quiet hours.

**Testing:** 159 Vitest tests (pure logic). Every migration verified on Postgres 16, idempotent (0008/0009/0010 RLS isolation checked end-to-end). Migrations are also mirrored as one-file `supabase/phase*.sql` for the owner to paste-run.
...
**Owner action needed:**
1. Run these three SQL files once each in Supabase → SQL Editor: `supabase/phase3_coach_prefs.sql` (0008, dashboard layouts), `supabase/phase4_messages.sql` (0009, chat), `supabase/phase4_notifications.sql` (0010, in-app notifications + nudges). Until run, those surfaces load but read/write errors.
2. *(Later, for PWA push + the 3-day re-engagement email — not built yet)* generate Web Push **VAPID keys** and pick an **email provider** (e.g. Resend) API key; these become env vars. In-app notifications + nudges work today without them.
