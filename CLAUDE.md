...
### Phase 7 — Weekly reports (§12) ✅ (in-app; scheduled notification is a cron follow-up)
...
- **Notification:** the in-app surfaces are live now; a scheduled "your recap is ready" notification can hook the existing daily engagement cron on a weekly cadence (follow-up).
...
- **Owner-gated to finish (can't be verified from the build sandbox):** create developer apps per provider, set `OURA_/FITBIT_/WHOOP_CLIENT_ID` + `_CLIENT_SECRET` in Vercel, register the callback URLs, then run a real **connect** verification. The **periodic step/sleep data-pull** (per-provider API mapping → steps into habits) is the remaining slice on top of this framework and needs live accounts to build+verify — not claimed done.
