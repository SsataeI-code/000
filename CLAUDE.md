...
- **Plate builder** (teaching/visualization tool; `plate.ts` pure/tested): `plateTotals(counts)` turns hand-portion counts (palms/cupped/thumbs/fists) into approximate macros+calories (clearly "a learning estimate — not a food log"); `macroShares` for the visual. `PlateBuilder` (`/client/plate`, linked from `/client/food`): tap portions up/down, a flat SVG plate fills **by volume** (so veggies visibly take half — teaches the plate method, no gradients), live macro/calorie totals + % of the client's day. Barcode/scale-free way to learn balance.
