...
- **Movement lives here** — e.g. "30-min workout," "1-hour bike ride," "8k steps," "hydration." **No workout programming or exercise library.** *(Owner decision, 2026: a simple **lift-progress log** IS now included — a spreadsheet-style record of exercise · weight · reps · sets with an estimated 1RM and personal bests, so clients can track strength over time and the coach can see it. Still not a full training-program builder.)*
...
- **Diet patterns & food preferences** (client-customizable; migration `0025_diet_prefs`, mirrored `supabase/phase-diet.sql`; `src/lib/food/diet.ts` pure/tested): `diet_pattern` (anything / vegetarian / vegan / pescatarian / mediterranean / carnivore) + free-text `food_avoid` on `client_profiles`. `fitsDiet`/`isAvoided`/`allowedByDiet` classify a food by name (keyword heuristic; plural-tolerant avoid match) and filter **recommendations only** — the full catalog stays searchable/loggable. Wired into `suggestFills` **and** `suggestMeals` (a meal is suggested only if every ingredient fits). Set in onboarding (diet selector + "anything to avoid?") and editable anytime via `FoodPreferences` on `/client/food` (`saveFoodPrefsAction`); coach can also write it (`client_profiles_write`).
- **Familiar-first + one-tap suggestions** (§5B): `suggestFills` takes `recentNames` (from `getRecentFoodNames`, distinct names logged in the last 30d) and gently boosts foods the client has eaten before (1.25×) so suggestions lean on the familiar while a much-richer new food still surfaces; a "You've had this" tag marks them. `FoodPick` now carries the serving's macros + per-100g map, so **`LogSuggestionButton` logs a suggestion in one tap** (`logFoodAction`, micros scaled) right from Fill-your-rings.
...
- **Plate builder** (teaching/visualization tool; `plate.ts` pure/tested): `plateTotals(counts)` turns hand-portion counts (palms/cupped/thumbs/fists) into approximate macros+calories (clearly "a learning estimate — not a food log"); `macroShares` for the visual. `PlateBuilder` (`/client/plate`, linked from `/client/food`): tap portions up/down, a flat SVG plate fills **by volume** (so veggies visibly take half — teaches the plate method, no gradients), live macro/calorie totals + % of the client's day. Barcode/scale-free way to learn balance. **Log the plate:** a "Log this plate" button saves the built plate's approximate macros/calories to today's food log via `logFoodAction` (name "Plate — hand portions", source manual) — so the teaching tool doubles as a fast, scale-free way to log a meal.
- **Coach/owner can use the client app** (owner request "use the site even as the owner"): `canAccessArea(role, "client")` now returns true for every signed-in role (was client-only), so a coach or the owner can open `/client` to track their own habits/food and dogfood the experience (their own `client_id=auth.uid()` data; onboarding runs for them like any client). Cross-nav: a **"Use the app"** link on the coach dashboard → `/client`, and a **"Coach view"** link in the client header (shown only to `hasCoachPowers`) → `/coach`. Middleware + client layout share the same `canAccessArea` gate, so both open together.
- **Barcode scanner fix:** `@undecaf/zbar-wasm` loads `zbar.wasm` by a package-relative URL that 404s under Next's bundling, so decodes failed silently (camera opened, never scanned). Now the wasm is served from `/public` (`public/zbar.wasm`, kept in sync by a `prebuild` copy script) and `BarcodeScanner` calls `setModuleArgs({ locateFile: () => "/zbar.wasm" })` + forces `getInstance()` up front so a load failure surfaces a clear message instead of failing frame-by-frame.
...
- **Per-client strictness** (§B "per-client strictness setting, coach-controlled"; migration `0023_client_strictness`, mirrored `supabase/phase-strictness.sql`; `src/lib/nutrition/strictness.ts` pure/tested): `precise` / `protein_cals` / `flexible` / `habits_only`, stored on `client_profiles.strictness` (survives target recalcs; coach writes it via the existing `client_profiles_write`/`is_coach_of` policy). `coachSetStrictnessAction` + a selector in `ClientPlanTools`. The client Today screen adapts: **habits_only** hides the macro rings (shows a "Habits-focused — no macro targets" note) + drops fill-your-rings and micros; **flexible** adds a "ranges are fine, ±10%" line; **protein_cals** adds a "hit protein & calories, carbs/fat flexible" line; **precise** is exact (unchanged). Verified idempotent on Postgres 16.
...
- **Remove a client** (§13 departed-client handling; `archiveClientAction` + `deleteClientAction` in `coach/actions.ts`; `RemoveClient` on the deep-dive): **Archive** (any coach, reversible, two-step confirm) flips the client's active `coach_clients` link to `archived` — they drop off every roster/queue/stat immediately (all roster queries filter `status='active'`), but their data is retained per the 1-year rule and they can be re-added later (the one-active-coach unique index is active-only). **Permanent delete** (irreversible, type-DELETE confirm) removes the client via the service-role `auth.admin.deleteUser` — every table cascades from `auth.users → profiles → logs/habits/messages`. Both authorized server-side to the client's own coach or the owner (`authorize()` = owner or `coachHasClient`); delete runs through a `delete_client(uuid)` SECURITY DEFINER RPC (migration `0024_delete_client_rpc`, mirrored `supabase/phase-delete-client.sql`) that authz-checks `is_owner()`/`is_coach_of` then deletes `auth.users` (cascades to profiles + all data) — **no service-role key needed**. Delete now shows for whoever can manage the client, not just the owner (fix: a coach couldn't delete their own clients because the button was owner-gated). Redirect to the roster on success. Verified on Postgres 16 (archive drops from the active roster and re-add succeeds). **Archived-clients view** (`/coach/archived`, `getArchivedClients` + `ArchivedClients`/`restoreClientAction`): lists archived clients with a one-tap **Restore** (flips `archived → active`, guarded against the one-active-coach rule — a clear message if the client already picked up another coach). Linked from an "Archived" header link on the roster; owner sees everyone's archived links.
...
- **Coach deep-dive panel:** `getWearableDaily(clientId, 7)` + `WearableSummary` render a client's recent synced steps/sleep on `/coach/clients/[id]` (only when rows exist — no empty shell before a tracker connects). Coach/owner read via the verified `is_coach_of`/owner RLS.
...
### Lift-progress tracker (owner decision, 2026) ✅

- **Strength log** (migration `0026_lifts.sql`, mirrored `supabase/phase-lifts.sql`): `lift_logs` — one row per set-record (exercise · weight · unit · reps · sets · date · note). RLS mirrors body data: the client owns it; their coach + owner read it (verified idempotent + 4 policies on Postgres 16). `src/lib/lifts/progress.ts` (pure/tested): Epley **estimated 1RM**, per-exercise **personal bests** (top weight + best e1RM, lb-normalized), distinct-exercise list, PR detection. `data.ts`/`actions.ts` (log/delete, defensive validation, client-local `log_date`). **Spreadsheet-style UI** `/client/lifts` (`LiftTracker`): add-a-set form (exercise datalist of past lifts, weight + lb/kg toggle, reps, sets), a personal-bests strip, and a scrollable history grid with e1RM per set. Linked from `/client/body`. Coach deep-dive shows the client's lifts **read-only** (bests + history) when any exist.

**Testing:** 297 Vitest tests (pure logic). Every migration verified on Postgres 16, idempotent (0008/0009/0010 RLS isolation checked end-to-end; 0011 structure verified; **0013 referrals** — self-referral ignored + RLS/authz; **0014 content** — owner-write/all-read + RLS — both checked end-to-end). Migrations are also mirrored as one-file `supabase/phase*.sql` for the owner to paste-run.
